...
void CBPTC19696::decodeErrorCheck()
{
	...
	do
	{
		...
		bool col[13U];
		for (unsigned c = 0U; c < 15U; c++)
		{
			unsigned pos = c + 1U;
			for (unsigned a = 0U; a < 13U; a++)
			{
		...
		for (unsigned r = 0U; r < 9U; r++)
		{
			unsigned pos = (r * 15U) + 1U;
...
void CBPTC19696::decodeExtractData(uint8_t *data) const
{
	bool bData[96U];
	unsigned pos = 0U;
	for (unsigned a = 4U; a <= 11U; a++, pos++)
		bData[pos] = m_deInterData[a];

	for (unsigned a = 16U; a <= 26U; a++, pos++)
		bData[pos] = m_deInterData[a];

	for (unsigned a = 31U; a <= 41U; a++, pos++)
		bData[pos] = m_deInterData[a];

	for (unsigned a = 46U; a <= 56U; a++, pos++)
		bData[pos] = m_deInterData[a];

	for (unsigned a = 61U; a <= 71U; a++, pos++)
		bData[pos] = m_deInterData[a];

	for (unsigned a = 76U; a <= 86U; a++, pos++)
		bData[pos] = m_deInterData[a];

	for (unsigned a = 91U; a <= 101U; a++, pos++)
		bData[pos] = m_deInterData[a];

	for (unsigned a = 106U; a <= 116U; a++, pos++)
		bData[pos] = m_deInterData[a];

	for (unsigned a = 121U; a <= 131U; a++, pos++)
		bData[pos] = m_deInterData[a];
