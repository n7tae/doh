...
void CSHA256::conclude()
{
	...
	unsigned bytes = m_buflen;
	unsigned size = (bytes < 56) ? 64 / 4 : 64 * 2 / 4;
...
/* Compute SHA256 message digest for LEN bytes beginning at BUFFER.  The
   ...
   digest.  */
uint8_t *CSHA256::buffer(const uint8_t *buffer, unsigned len, uint8_t *resblock)
{
	assert(buffer != NULL);
...
void CSHA256::processBytes(const uint8_t *buffer, unsigned len)
{
	assert(buffer != NULL);
	...
	if (m_buflen != 0U)
	{
		unsigned left_over = m_buflen;
		unsigned add = 128U - left_over > len ? len : 128U - left_over;
...
//# define UNALIGNED_P(p) (((unsigned) p) % alignof (uint32_t) != 0)
...
void CSHA256::processBlock(const uint8_t *buffer, unsigned len)
{
	assert(buffer != NULL);

	const uint32_t* words = (uint32_t*)buffer;
	unsigned nwords = len / sizeof(uint32_t);
