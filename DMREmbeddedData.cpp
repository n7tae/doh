...
bool CDMREmbeddedData::addData(const uint8_t *data, uint8_t lcss)
{
	assert(data != NULL);
...
void CDMREmbeddedData::encodeEmbeddedData()
{
	...
	unsigned b = 0U;
	for (unsigned a = 0U; a < 11U; a++, b++)
		data[a] = m_data[b];
	for (unsigned a = 16U; a < 27U; a++, b++)
		data[a] = m_data[b];
	for (unsigned a = 32U; a < 42U; a++, b++)
		data[a] = m_data[b];
	for (unsigned a = 48U; a < 58U; a++, b++)
		data[a] = m_data[b];
	for (unsigned a = 64U; a < 74U; a++, b++)
		data[a] = m_data[b];
	for (unsigned a = 80U; a < 90U; a++, b++)
		data[a] = m_data[b];
	for (unsigned a = 96U; a < 106U; a++, b++)
		data[a] = m_data[b];

	// Hamming (16,11,4) check each row except the last one
	for (unsigned a = 0U; a < 112U; a += 16U)
		CHamming::encode16114(data + a);

	// Add the parity bits for each column
	for (unsigned a = 0U; a < 16U; a++)
...
uint8_t CDMREmbeddedData::getData(uint8_t *data, uint8_t n) const
{
	assert(data != NULL);
...
bool CDMREmbeddedData::getRawData(uint8_t *data) const
{
	assert(data != NULL);
